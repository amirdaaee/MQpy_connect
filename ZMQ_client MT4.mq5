...
void WhichAction(CJAVal &resp_json)
  {
   ...
   if(command=="position_open") PositionOpen(&resp_json);
   else if(command=="position_modify") PositionModify(&resp_json);
   else if(command=="position_close") PositionClose(&resp_json);
   else if(command=="resume") return;
   else if(command=="error") Print("server side error");
   else Print("command not understood");
  }
//+------------------------------------------------------------------+
//| Action
//+------------------------------------------------------------------+
void PositionOpen(CJAVal &resp_json)
  {
     int POS = OrderSend ( Symbol() , OP_SELLSTOP , lot , Bid-Pending*Point , 100 , Bid-(Pending-Stoploss)*Point , 0 , "Strategy ZigZag" , MagicSell , 0 , clrRed ) ;
...
// fulfill from response
   ...
   double MTrequest.volume=resp_json["args"]["volume"].ToDbl();
...
// order allocation
   int POS = OrderSend ( Symbol() , MTrequest.action , MTrequest.volume , Bid-Pending*Point , 100 , Bid-(Pending-Stoploss)*Point , 0 , "Strategy ZigZag" , MagicNumber , 0 , clrRed ) ;
   if(status==true)
     {
